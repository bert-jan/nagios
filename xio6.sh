...
print_help() {
    echo "Usage: $0 [-u <url>] [-U <username>] [-P <password>] [-l <latency_threshold>] [-w <bw_warning>] [-c <bw_critical>] [-i]"
    echo ""
    echo "Required arguments:"
    echo "  -u <url>                  The URL of the XtremIO API to fetch statistics from (e.g. https://your_xtremio_host/api/1.4/system/statistics)"
    echo "  -U <username>             The username for XtremIO authentication."
    echo "  -P <password>             The password for XtremIO authentication."
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  ./check_xtremio.sh -u 'https://your_xtremio_host/api/1.4/system/statistics' -U 'your_username' -P 'your_password' -l 5"
    echo "  ./check_xtremio.sh -u 'https://your_xtremio_host/api/1.4/system/statistics' -U 'your_username' -P 'your_password' -w 0.5 -c 1.0"
    echo "  ./check_xtremio.sh -u 'https://your_xtremio_host/api/1.4/system/statistics' -U 'your_username' -P 'your_password' -i"
}
...
while getopts "u:U:P:l:w:c:i" opt; do
  case ${opt} in
    u ) HOST_URL=$OPTARG ;;  # Host URL
    U ) USER=$OPTARG ;;  # Username
    P ) PASS=$OPTARG ;;  # Password
...
# Controleren of alle vereiste argumenten zijn opgegeven
if [ -z "$HOST_URL" ]; then
